...
#define EG_ASSERT_MALLOC(n)                                                    \
  {                                                                            \
    ...
    b = EG_Malloc_CanFindAllocationInList(n);                                  \
                                                                               \
    if (!b) {                                                                  \
      EG_Log(EG_LOG_DEBUG5, dL " Assertion FAILED for " EG_PTRFMT "", dR,      \
...
#define EG_VASSERT_MALLOC(n, s)                                                \
  {                                                                            \
    ...
    b = EG_Malloc_CanFindAllocationInList(n);                                  \
                                                                               \
    if (!b) {                                                                  \
      EG_Log(EG_LOG_DEBUG5, dL " Assertion FAILED for " EG_PTRFMT ": %s", dR,  \
...
#define EG_VASSERT_MALLOC(n, s)                                                \
  {                                                                            \
    ...
    if (!b) {                                                                  \
      EG_Log(EG_LOG_DEBUG5, dL "Assertion FAILED for " EG_PTRFMT ": %s", dR,   \
             (b ? "PASSED" : "FAILED"), EG_CAST_PTR_TO_INT(n), message);       \
    }                                                                          \
  }
#endif
#else
