...
void UserVIAWrite(int Address, int Value) {
  /* cerr << "UserVIAWrite: Address=0x" << hex << Address << " Value=0x" <<
  Value << dec << " at " << TotalCycles << "\n"; DumpRegs(); */

  if (DebugEnabled) {
    char info[200];
    sprintf(info, "UsrVia: Write address %X value %02X", (int)(Address & 0xf),
            Value & 0xff);
  ...
  case 1:
    ...
    if (PrinterEnabled) {
      if (fputc(UserVIAState.ora, PrinterFileHandle) == EOF) {
        ...
        //--#else
        //--          cerr << "Failed to write to printer file " <<
        //PrinterFileName << "\n";
        //--#endif
      } else {
  ...
  case 4:
  case 6:
    /*cerr << "UserVia Reg4 Timer1 lo Counter Write val=0x " << hex << Value <<
     * dec << " at " << TotalCycles << "\n"; */
  ...
  case 5:
    /*cerr << "UserVia Reg5 Timer1 hi Counter Write val=0x" << hex << Value <<
     * dec  << " at " << TotalCycles << "\n"; */
  ...
  case 7:
    ...
    UserVIAState.timer1l |= (Value & 0xff) << 8;
    UserVIAState.ifr &=
        0xbf; /* clear timer 1 ifr (this is what Model-B does) */
    UpdateIFRTopBit();
    break;

  case 8:
    /* cerr << "UserVia Reg8 Timer2 lo Counter Write val=0x" << hex << Value <<
     * dec << "\n"; */
  ...
  case 9:
    /* cerr << "UserVia Reg9 Timer2 hi Counter Write val=0x" << hex << Value <<
    dec << "\n"; core_dumpstate(); */
...
int UserVIARead(int Address) {
  int tmp = 0xff;
  /* cerr << "UserVIARead: Address=0x" << hex << Address << dec << " at " <<
  TotalCycles << "\n"; DumpRegs(); */

  switch (Address) {
  case 0: /* IRB read */
    tmp = (UserVIAState.orb & UserVIAState.ddrb) |
  ...
  if (DebugEnabled) {
    char info[200];
    sprintf(info, "UsrVia: Read address %X value %02X", (int)(Address & 0xf),
            tmp & 0xff);
    DebugDisplayTrace(DEBUG_USERVIA, true, info);
  }
...
void DebugUserViaState() { DebugViaState("UsrVia", &UserVIAState); }
...

{
  ...
  //--	{
  //--		hwndBreakOut = CreateDialog(hinst,
  //MAKEINTRESOURCE(IDD_BREAKOUT),
  //--										NULL,
  //(DLGPROC)BreakOutDlgProc);
...
//--			{
//--				if (bit == 1) UserVIAState.irb &= ~0x80; else
//UserVIAState.irb |= 0x80;
//--			}
//--			break;
//--
//--		case IDC_IB6:
...
//--			{
//--				if (bit == 1) UserVIAState.irb &= ~0x40; else
//UserVIAState.irb |= 0x40;
//--			}
//--			break;
//--
//--		case IDC_IB5:
...
//--			{
//--				if (bit == 1) UserVIAState.irb &= ~0x20; else
//UserVIAState.irb |= 0x20;
//--			}
//--			break;
//--
//--		case IDC_IB4:
...
//--			{
//--				if (bit == 1) UserVIAState.irb &= ~0x10; else
//UserVIAState.irb |= 0x10;
//--			}
//--			break;
//--
//--		case IDC_IB3:
...
//--			{
//--				if (bit == 1) UserVIAState.irb &= ~0x08; else
//UserVIAState.irb |= 0x08;
//--			}
//--			break;
//--
//--		case IDC_IB2:
...
//--			{
//--				if (bit == 1) UserVIAState.irb &= ~0x04; else
//UserVIAState.irb |= 0x04;
//--			}
//--			break;
//--
...
//--			{
//--				if (bit == 1) UserVIAState.irb &= ~0x02; else
//UserVIAState.irb |= 0x02;
//--			}
//--			break;
//--
//--		case IDC_IB0:
...
//--			{
//--				if (bit == 1) UserVIAState.irb &= ~0x01; else
//UserVIAState.irb |= 0x01;
//--			}
//--			break;
//--
...
{
  return 0;
  //--	return (SendDlgItemMessage(hwndBreakOut, ctrlID, BM_GETCHECK, 0, 0) ==
  //BST_CHECKED);
}
...
//--LRESULT CALLBACK GetBitKeyWndProc( HWND hWnd,		   // window
//handle
//--								UINT message,	   //
//type of message
//--								WPARAM uParam,	   //
