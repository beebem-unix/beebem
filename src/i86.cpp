...
typedef struct {
  ...
  int AuxVal, OverVal, SignVal, ZeroVal, CarryVal,
      DirVal; /* 0 or non-0 valued flags */
  UINT8 ParityVal;
...
struct i86_timing {
  ...
  UINT8 alu_r16i8, alu_m16i8,
      alu_m16i8_ro; /* ALU ops, 16-bit w/8-bit immediate */
  ...
  UINT8 enter0, enter1, enter_base, enter_count,
      leave;   /* (80186) ENTER/LEAVE */
...
static void PREFIX86(_interrupt)(unsigned int_num) {
  ...
  //    WriteLog("[%04x:%04x] AX=%04x BX=%04x CX=%04x DX=%04x SI=%04x DI=%04x
  //    DS=%04x ES=%04x SS:SP = %04x:%04x",
  //        I.sregs[CS], (I.pc - I.base[CS]) & 0xffff,
  //        I.regs.w[AX], I.regs.w[BX], I.regs.w[CX], I.regs.w[DX],
  //        I.regs.w[SI], I.regs.w[DI], I.sregs[DS], I.sregs[ES],
...
static void PREFIX86(_rotate_shift_Byte)(unsigned ModRM, unsigned count) {
  ...
  if (count == 0) {
    ...
    ICOUNT -= (ModRM >= 0xc0) ? cycles.rot_reg_base + cycles.rot_reg_bit
                              : cycles.rot_m8_base + cycles.rot_m8_bit;
...
static void PREFIX86(_rotate_shift_Word)(unsigned ModRM, unsigned count) {
  ...
  if (count == 0) {
    ...
    ICOUNT -= (ModRM >= 0xc0) ? cycles.rot_reg_base + cycles.rot_reg_bit
                              : cycles.rot_m8_base + cycles.rot_m16_bit;
...
static void PREFIX86(_escape)(
    void) /* Opcodes 0xd8, 0xd9, 0xda, 0xdb, 0xdc, 0xdd, 0xde and 0xdf */
{
  unsigned ModRM = FETCH;
  ICOUNT -= cycles.nop;
  GetRMByte(ModRM);
...
static void PREFIX186(_imul_d16)(void) /* Opcode 0x69 */
{
  ...
  I.CarryVal = I.OverVal =
      (((INT32)dst) >> 15 != 0) && (((INT32)dst) >> 15 != -1);
...

{
  ...
  sprintf(buff,
          "[%04x:%04x] AX=%04x BX=%04x CX=%04x DX=%04x SI=%04x DI=%04x DS=%04x "
          "ES=%04x SS:SP = %04x:%04x ",
          I.sregs[CS], (I.pc - I.base[CS]) & 0xffff, I.regs.w[AX], I.regs.w[BX],
          I.regs.w[CX], I.regs.w[DX], I.regs.w[SI], I.regs.w[DI], I.sregs[DS],
          I.sregs[ES], I.sregs[SS], I.regs.w[SP]);
...
offs_t i186_dasm(char *buffer, offs_t pc, UINT8 *oprom, UINT8 *opram,
                 int bytes) {
...
extern void WriteTubeFromParasiteSide(unsigned char IOAddr,
