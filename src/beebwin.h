...
enum TextToSpeechSearchDirection { TTS_FORWARDS, TTS_BACKWARDS };
...
public:
  ...
  */
  ...
  EightUChars *GetLinePtr(int y) {
    ...
    //--			return((EightUChars
    //*)(m_screen+(MAX_VIDEO_SCAN_LINES*800)));
  ...
  char *imageData(void) {
    //->		return
    //(m_screen+ScreenAdjust>m_screen)?m_screen+ScreenAdjust:m_screen;
    //++
    char *p = NULL;
...
private:
  ...
  // object
  // LPDIRECTDRAW2			m_DD2;			// DirectDraw
  ...
  // LPDIRECTDRAWCLIPPER		m_Clipper;		// clipper for
  // primary
