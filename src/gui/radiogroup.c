...
static void Callback_Destroy(EG_Widget *widget_ptr) {
  ...
  if (EG_Widget_GetType(widget_ptr) != EG_Widget_Type_RadioGroup) {
     ...
     * Radio Buttons again).
     */
    if ((radiogroup_ptr = (EG_RadioGroup *)EG_Widget_GetPayload(widget_ptr)) !=
        NULL) {
      for (i = 0; i < radiogroup_ptr->count; i++) {
        ...
        default:
          EG_Log(EG_LOG_ERROR, dL "Widget %s is of incorrect type.", dR,
...
static EG_BOOL Callback_Visible(EG_Widget *widget_ptr, EG_BOOL is_visible) {
  ...
  // [TODO] THIS ONE SHOULD BE OK FOR THE CHANGE, EG_Shared_UpdateVisibleState
  // DOES ALL THE REPAINTING/FOCUS WORK.
...
EG_Widget *EG_RadioGroup_Create(const char *name_ptr) {
  ...
  SHARED__CREATE_NEW_EG_WIDGET(widget_ptr, name_ptr, EG_Widget_Type_RadioGroup,
                               radiogroup_ptr);
...
EG_BOOL EG_RadioGroup_AddButton(EG_Widget *group_widget_ptr,
  ...
  case EG_Widget_Type_RadioButton:
    /* Attach the button
     */
    radiogroup_ptr->button_widget_ptr[radiogroup_ptr->count++] =
        button_widget_ptr;

    /* Associate the radio button with this radio group.
     */
    if (EG_RadioButton_SetGroupParent(button_widget_ptr, group_widget_ptr) !=
        EG_TRUE)
      EG_Log(EG_LOG_ERROR,
             dL "Could not associate EG_RadioButton with EG_RadioGroup.\n", dR);
  ...
  case EG_Widget_Type_ToggleButton:
     ...
     */
    if (EG_ToggleButton_SetGroupParent(button_widget_ptr, group_widget_ptr) !=
        EG_TRUE)
      EG_Log(EG_LOG_ERROR,
             dL "Could not associate EG_ToggleButton with EG_RadioGroup.\n",
             dR);
...
void EG_RadioGroup_RemoveButton(EG_Widget *group_widget_ptr,
   ...
   */
  for (i = 0; i < radiogroup_ptr->count; i++)
    if (EG_Widget_GetID(button_widget_ptr) ==
        EG_Widget_GetID(radiogroup_ptr->button_widget_ptr[i]))
      break;
  ...
  // [TODO] If the removed widget had focus, move focus, if still have focus,
  // clear focus. [TODO] If this button was selected make next button selected.
...
EG_BOOL EG_RadioGroup_Select(EG_Widget *button_widget_ptr) {
  ...
  switch (EG_Widget_GetType(button_widget_ptr)) {
  case EG_Widget_Type_RadioButton:
    if ((group_widget_ptr = EG_RadioButton_GetGroupParent(button_widget_ptr)) ==
        NULL) {
      EG_Log(EG_LOG_ERROR,
  ...
  case EG_Widget_Type_ToggleButton:
    if ((group_widget_ptr =
             EG_ToggleButton_GetGroupParent(button_widget_ptr)) == NULL) {
      EG_Log(EG_LOG_ERROR,
    ...
    break;
  default:
    /* To keep gcc happy, the switch above will make sure this is never called
     * anyway.
     */
