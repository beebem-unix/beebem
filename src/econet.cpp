...
// there are no start/stop bits, however to avoid detecting a dead line as
// ffffff zeros are added and removed transparently if you get more than five
// "1"s during data transmission - more than 5 are flags or errors) 6854
// datasheet has max clock frequency of 1.5MHz for the B version. 64 cycles
// seems to be a bit fast for 'netmon' prog to keep up - set to 128.
...
void EconetReset(void) {
  ...
  ADLC.status1 =
      0; // cts - clear to send line input (no collissions talking udp)
  ...
  //--		sprintf(info, "Econet: Failed to open listening socket (error
  //%ld)", WSAGetLastError());
  //--		EconetError(info);
  //--		WSACleanup();
  //--		return;
  ...
  if (EconetStationNumber != 0) {
    ...
    if (EconetListenPort != 0) {
      ...
      //--			}
      //++
      if (bind(ListenSocket, (struct sockaddr *)&service, sizeof(service)) ==
          -1) {
      ...
      //--			return;
      //++
      sprintf(info, "Failed to find station %d in econet.cfg",
              EconetStationNumber);
      EconetError(info);
      return;
      //<-
    ...
    if (gethostname(localhost, 256) != -1 &&
      ...
      for (unsigned int i = 0; i < networkp && EconetStationNumber == 0; ++i) {
        ...
        for (int a = 0;
             hent->h_addr_list[a] != NULL && EconetStationNumber == 0; ++a) {
  ...
  //--	}
  //++
  pDEBUG(dL "Station number %d, port %d", dR, EconetStationNumber,
         EconetListenPort);
  //<-
  ...
  //-- 		sprintf(info, "Econet: Failed to open sending socket (error
  //%ld)", WSAGetLastError());
  //--		EconetError(info);
  //--		closesocket(ListenSocket);
  //--		WSACleanup();
...
unsigned char Read_Econet_Station(void) {
  //->	if (DebugEnabled) {
  //--		char info[200];
  //--		sprintf(info, "Econet: Read Station %02X",
  ...
  if (Register > 1) {
    if (((ADLC.control1 & 64) == 0) &&
        ADLC.rxfptr) { // rxreset not set and someting in fifo
                       ...
                       //--			}
                       //++
      pDEBUG(dL "Econet returned FIFO:  %02X", dR,
             (int)ADLC.rxfifo[ADLC.rxfptr - 1]);
      debugADLCprint();
      //<-
      if (ADLC.rxfptr) {
...
void WriteEconetRegister(unsigned char Register, unsigned char Value) {
  //->	if (DebugEnabled) {
  //--		char info[200];
  //--		sprintf(info, "Econet: Write ADLC %02X = %02X", (int)Register,
  //(int)Value);
  //--		DebugDisplayTrace(DEBUG_ECONET, true, info);
  //--	}
  //++
  ...
  if (Register == 0) { // adr 00
    ...
    // can also finalise tx by setting a control bit.so do that automatically
    // for reg 3 worry about actually sending stuff in the poll routines, not
    // here.
...
bool EconetPoll_real(void) { // return NMI status
  ...
  // okie dokie.  This is where the brunt of the ADLC emulation & network
  ...
  // CR1b1 - RIE - Receiver Interrupt Enable - Flag to allow receiver section to
  // create interrupt.
  //		no action needed here
  ...
  // CR2b1 - 2byte/1byte mode.  set to indicate 2 byte mode. see trda status
  // bit. CR2b2 - Flag/Mark idle select. What is transmitted when tx idle.
  // ignored here as not needed CR2b3 - FC/TDRA mode - does status bit SR1b6
  // indicate 1=frame complete,
  ...
  // CR3b0 - LCF - Logical Control Field Select. if zero, no control fields in
  // frame, ignored. CR3b1 - CEX - Extend Control Field Select - when set,
  // control field is 16 bits. ignored. CR3b2 - AEX - When set, address will be
  ...
  // directly. pin not used in a BBC B

  // -----------------------
  // CR4b0 - FF/F - when clear, re-used the Flag at end of one packet as start
  // of next packet. ignored. CR4b1,2 - TX word length. 11=8 bits. BBC uses 8
  // bits so ignore flags and assume 8 bits throughout CR4b3,4 - RX word length.
  // 11=8 bits. BBC uses 8 bits so ignore flags and assume 8 bits throughout
  ...
  if (EconetTrigger <= TotalCycles) {
    ...
    if (!(ADLC.control1 & 128)) { // tx reset off
      if (ADLC.txfptr) {          // there is data is in tx fifo
        ...
        if (EconetTxWritePointer + 1 >
                sizeof(Econettxbuff) || // overflow IP buffer
    ...
    if (!(ADLC.control1 & 64)) { // rx reset off
      if (EconetRxReadPointer < EconetRxBytesInBuffer) {
        ...
        if (ADLC.rxfptr <
          ...
          EconetRxReadPointer++;
          if (EconetRxReadPointer >=
              EconetRxBytesInBuffer) { // that was last byte!
      ...
      if (ADLC.rxfptr == 0) {
        ...
        if (!(ADLC.status2 & 2)) {
          ...
          if (RetVal > 0) {
            // Read the packet
            RetVal = recv(ListenSocket, (char *)Econetrxbuff,
                          sizeof(Econetrxbuff), 0);
            if (RetVal > 0) {
              //->							if
              //(DebugEnabled) {
              //--
              ...
              if (Econetrxbuff[0] == EconetStationNumber) {
                ...
                // Two other stations communicating - assume one of them will
                // flag fill
                FlagFillActive = true;
                SetTrigger(EconetFlagFillTimeout, EconetFlagFillTimeoutTrigger);
                //-- if (DebugEnabled) DebugDisplayTrace(DEBUG_ECONET, true,
  ...
  if (ADLC.control2 & 1) {
    if ((ADLC.sr2pse <= 1) &&
        (ADLC.status2 & 0x7A)) { // ERR, FV, DCD, OVRN, ABT
      ADLC.sr2pse = 1;
      ADLC.status2 &= ~0x85;
  ...
  if (ADLC.status1 != ADLCtemp.status1 ||
      ADLC.status2 != ADLCtemp.status2) { // something changed
...
void debugADLCprint(void) {
  char info[200];
  sprintf(info,
          "ADLC: Ctrl:%02X %02X %02X %02X St:%02X %02X TXFptr:%02x rx:%02x "
          "FF:%d IRQc:%02x SR2c:%02x PC:%04x  ",
          (int)ADLC.control1, (int)ADLC.control2, (int)ADLC.control3,
          (int)ADLC.control4, (int)ADLC.status1, (int)ADLC.status2,
