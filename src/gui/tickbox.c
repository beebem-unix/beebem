...
static EG_BOOL Callback_Paint(EG_Widget *widget_ptr, SDL_Rect area) {
  ...
  if (EG_Shared_GetRenderingDetails(widget_ptr, area, (void *)&tickbox_ptr,
...
static EG_BOOL Callback_SDL_Event(EG_Widget *widget_ptr, SDL_Event *event_ptr) {
   ...
   * false.
   */
  if (EG_Shared_GetEventDetails(widget_ptr, event_ptr, (void *)&tickbox_ptr,
  ...
  if (event_ptr->type == SDL_MOUSEBUTTONDOWN ||
     ...
     */
    if (tickbox_ptr->depressed == EG_FALSE &&
    ...
    if (tickbox_ptr->depressed == EG_TRUE &&
      ...
      if (mouse_over_widget == EG_TRUE &&
        ...
        if (tickbox_ptr->parent_group_widget != NULL) {
           ...
           * selected, select it.
           */
          // *** If we dont call EG_RadioGroup_Select every time then focus will
          // not change..
