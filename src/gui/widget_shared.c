 ...
 * too.
 */
EG_BOOL EG_Callback_Generic_SDL_Event(EG_Widget *widget_ptr,
                                      SDL_Event *event_ptr) {
...
/* V I S I B L E:
 *
 * When visibility state changes, the callback will re-render the widget or
 * screen itself.
 */
 ...
 */
EG_BOOL EG_Callback_Generic_Enabled_NoSupport(EG_Widget *widget_ptr,
                                              EG_BOOL is_enabled) {
 ...
 *
 */
EG_BOOL EG_Shared_GetRenderingDetails(
    EG_Widget *widget_ptr, SDL_Rect area,
...
#define overlap(a, b)                                                          \
  ...
  //
  //
  //#define overlap(a, b) ( ( ( ((a.y - (b.y+b.h) ) ^ ( (a.y+a.h) - b.y)) &
  //((a.x - (b.x+b.w) ) ^ ( (a.x+a.w) - b.x)) ) < 0) ? 1 : 0)
  //
  //	if ( overlap(a,b) )
  //		printf("OVERLAP\n");
  //	else
...
EG_BOOL EG_Shared_GetEventDetails(
    EG_Widget *widget_ptr, SDL_Event *event_ptr,
...
EG_BOOL EG_Shared_IsMouseOverArea(EG_Widget *widget_ptr, SDL_Rect wa,
                                  SDL_Event *event_ptr) {
...
EG_BOOL EG_Shared_IsMouseOverWidget(EG_Widget *widget_ptr,
                                    SDL_Event *event_ptr) {
  EG_Window *window_ptr;
  SDL_Rect wa;
  int x, y;
 ...
 * above).
 */
EG_BOOL EG_Shared_UpdateVisibleState(EG_Widget *widget_ptr,
                                     EG_BOOL is_visible) {
  EG_Window *window_ptr;
  SDL_Rect size;
 ...
 * above).
 */
EG_BOOL EG_Shared_UpdateEnabledState(EG_Widget *widget_ptr,
                                     EG_BOOL is_enabled) {
  EG_Window *window_ptr;
