...
static EG_BOOL Callback_Paint(EG_Widget *widget_ptr, SDL_Rect area) {
  ...
  if (EG_Shared_GetRenderingDetails(widget_ptr, area, (void *)&button_ptr,
...
static EG_BOOL Callback_SDL_Event(EG_Widget *widget_ptr, SDL_Event *event_ptr) {
   ...
   * false.
   */
  if (EG_Shared_GetEventDetails(widget_ptr, event_ptr, (void *)&button_ptr,
  ...
  if (event_ptr->type == SDL_MOUSEBUTTONDOWN ||
     ...
     */
    if (button_ptr->depressed == EG_FALSE &&
...
EG_Widget *EG_Button_Create(const char *name_ptr, SDL_Color color,
                            int alignment, const char *caption_ptr,
                            SDL_Rect dimension) {
  EG_Button *button_ptr;
