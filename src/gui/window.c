...
static EG_BOOL InitializeWindowStruct(EG_Window *window_ptr,
                                      const char *name_ptr);
...
void EG_Window_DestroyAllChildWidgets(EG_Window *window_ptr) {
  ...
  while (EG_Window_Child_GetCount(window_ptr) != 0) {
    ...
    if (tmp_widget_ptr != NULL) {
      ...
      EG_Widget_CallAttach(tmp_widget_ptr, EG_Window_Type_Window,
...
EG_BOOL EG_Window_Child_SetFocusedIndexByWidget(EG_Window *window_ptr,
                                                EG_Widget *widget_ptr) {
...
EG_BOOL EG_Window_RemoveWidget(EG_Window *window_ptr, void *widget_ptr) {
   ...
   */
  from_index =
      EG_Window_Child_GetIndexFromWidget(window_ptr, (EG_Widget *)widget_ptr);
  if (from_index == -1) {
    EG_Log(EG_LOG_ERROR,
...
static EG_BOOL MoveFocus(EG_Window *window_ptr, int direction) {
  ...
  do {
    ...
    if (EG_Widget_CallGotFocus(widget_ptr) == EG_TRUE) {
      if (EG_Window_Child_SetFocusedIndex(window_ptr, current_index) !=
          EG_TRUE) {
        EG_Log(EG_LOG_ERROR,
...
EG_BOOL EG_Window_Repaint(EG_Window *window_ptr, SDL_Rect area) {
  ...
  // area.h, size.x, size.y, size.w, size.h);
  ...
  if (area.x == 0 && area.y == 0 && area.w == size.w && area.h == size.h) {
    ...
    // printf("area = %d %d %d %d, size = %d %d %d %d\n", area.x, area.y,
    // area.w, area.h, size.x, size.y, size.w, size.h);
...
EG_BOOL EG_Window_ProcessEvent(EG_Window *window_ptr, SDL_Event *event_ptr,
  ...
  // [TODO] Once all widgets converted to widget_shared.c code, make mouse
  // coordinates relative to window coordinates so widget_shared.c code does not
  // have to add the window x,y ofset within the surface. (See
...
EG_BOOL EG_Window_Hide(EG_Window *window_ptr) {
  ...
  if (window_ptr->window_background_surface_ptr != NULL) {
    if (SDL_BlitSurface(window_ptr->window_background_surface_ptr, NULL,
                        window_ptr->surface_ptr, NULL) != 0)
      EG_Log(EG_LOG_ERROR,
             dL "Window failed to re-store"
                " background",
             dR);
    else
      SDL_UpdateRect(window_ptr->surface_ptr, 0, 0, window_ptr->surface_ptr->w,
...
EG_BOOL EG_Window_Show(EG_Window *window_ptr) {
  ...
  window_ptr->window_background_surface_ptr = SDL_ConvertSurface(
      window_ptr->surface_ptr, window_ptr->surface_ptr->format, SDL_SWSURFACE);
...
EG_BOOL EG_Window_SetFocusToThisWidget(EG_Widget *widget_ptr) {
  ...
  if (EG_Widget_CallGotFocus(widget_ptr) == EG_TRUE) {
     ...
     */
    if (EG_Window_Child_SetFocusedIndexByWidget(window_ptr, widget_ptr) !=
        EG_TRUE)
      EG_Log(EG_LOG_ERROR,
     ...
     */
    if (focused_widget_ptr != NULL) {
      if (EG_Window_Child_SetFocusedIndexByWidget(
              window_ptr, focused_widget_ptr) != EG_TRUE)
        EG_Log(EG_LOG_ERROR,
               dL "Window failed to switch focus"
                  " index to new child widget. This is a bug :-(",
               dR);
