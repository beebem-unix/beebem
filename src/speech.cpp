...
const static short k1table[0x20] = {
    (short)0x82C0, (short)0x8380, (short)0x83C0, (short)0x8440, (short)0x84C0,
    (short)0x8540, (short)0x8600, (short)0x8780, (short)0x8880, (short)0x8980,
    (short)0x8AC0, (short)0x8C00, (short)0x8D40, (short)0x8F00, (short)0x90C0,
    (short)0x92C0, (short)0x9900, (short)0xA140, (short)0xAB80, (short)0xB840,
    (short)0xC740, (short)0xD8C0, (short)0xEBC0, 0x0000,        0x1440,
    0x2740,        0x38C0,        0x47C0,        0x5480,        0x5EC0,
    0x6700,        0x6D40};

/* K2 is (5-bits -> 9 bits+sign, 2's comp. fractional (-1 < x < 1) */

const static short k2table[0x20] = {
    (short)0xAE00, (short)0xB480, (short)0xBB80, (short)0xC340, (short)0xCB80,
    (short)0xD440, (short)0xDDC0, (short)0xE780, (short)0xF180, (short)0xFBC0,
    0x0600,        0x1040,        0x1A40,        0x2400,        0x2D40,
    0x3600,        0x3E40,        0x45C0,        0x4CC0,        0x5300,
    0x5880,        0x5DC0,        0x6240,        0x6640,        0x69C0,
    0x6CC0,        0x6F80,        0x71C0,        0x73C0,        0x7580,
    0x7700,        0x7E80};
...
const static short k4table[0x10] = {
    ...
    0x1D00,        0x2B00,        0x3900,        0x4700,
    0x5500,        0x6300,        0x7100,        0x7E00};
...
const static short k6table[0x10] = {
    ...
    0x1900,        0x2400,        0x2F00,        0x3A00,
    0x4500,        0x5000,        0x5B00,        0x6600};
...
const static short k7table[0x10] = {
    ...
    0x1300,        0x1F00,        0x2B00,        0x3700,
    0x4300,        0x4F00,        0x5A00,        0x6600};

/* K8 is (3-bits -> 9 bits+sign, 2's comp. fractional (-1 < x < 1) */

const static short k8table[0x08] = {(short)0xC000, (short)0xD800, (short)0xF000,
                                    0x0700,        0x1F00,        0x3700,
                                    0x4F00,        0x6600};

/* K9 is (3-bits -> 9 bits+sign, 2's comp. fractional (-1 < x < 1) */

const static short k9table[0x08] = {(short)0xC000, (short)0xD400, (short)0xE800,
                                    (short)0xFC00, 0x1000,        0x2500,
                                    0x3900,        0x4D00};

/* K10 is (3-bits -> 9 bits+sign, 2's comp. fractional (-1 < x < 1) */

const static short k10table[0x08] = {
    (short)0xCD00, (short)0xDF00, (short)0xF100, 0x0400,
    0x1600,        0x2000,        0x3B00,        0x4D00};

/* chirp table */

static char chirptable[41] = {
    0x00,       0x2a,       (char)0xd4, 0x32,       (char)0xb2, 0x12,
    0x25,       0x14,       0x02,       (char)0xe1, (char)0xc5, 0x02,
    0x5f,       0x5a,       0x05,       0x0f,       0x26,       (char)0xfc,
    (char)0xa5, (char)0xa5, (char)0xd6, (char)0xdd, (char)0xdc, (char)0xfc,
    0x25,       0x2b,       0x22,       0x21,       0x0f,       (char)0xff,
    (char)0xf8, (char)0xee, (char)0xed, (char)0xef, (char)0xf7, (char)0xf6,
    (char)0xfa, 0x00,       0x03,       0x02,       0x01};
...
// static void tms5220_update(void *param, stream_sample_t **inputs,
...
int my_read(int count) {
  ...
  //	fprintf(stderr, "In my_read with addr = 0x%04x, phrom = 0x%02x, count =
  //%d", addr, phrom, count);
  ...
  while (count--) {
    ...
    val = (val << 1) |
          ((phrom_rom[phrom][addr] >> tms5220->chip->phrom_bits_taken) & 1);
...
void tms5220_update(unsigned char *buff, int length) {
  ...
  if (info->source_pos > 0) {
    ...
    while (length > 0 && info->source_pos < FRAC_ONE) {
      samp = (((INT32)prev * (INT32)(FRAC_ONE - info->source_pos)) +
              ((INT32)curr * (INT32)info->source_pos)) >>
             FRAC_BITS;
    ...
    if (info->source_pos >= FRAC_ONE)
      ...
      if (buffer - buff != len)
        fprintf(stderr,
                "Here for some reason - mismatch in num of samples = %d, %ld\n",
                len, buffer - buff);
...
void tms5220_set_frequency(int frequency) {
  //	tms5220->source_step = (UINT32)((double)(frequency / 80) *
  //(double)FRAC_ONE / (double) 44100.0);
  tms5220->source_step = (UINT32)((double)(frequency / 80) * (double)FRAC_ONE /
...
void tms5220_destroy(struct tms5220 *chip) { free(chip); }
...
void tms5220_reset_chip(struct tms5220 *chip) {
  ...
  tms->fifo_head = tms->fifo_tail = tms->fifo_count = tms->fifo_bits_taken =
      tms->phrom_bits_taken = 0;

  /* initialize the chip state */
  /* Note that we do not actually clear IRQ on start-up : IRQ is even raised if
   * tms->buffer_empty or tms->buffer_low are 0 */
  tms->tms5220_speaking = tms->speak_external = tms->talk_status =
      tms->first_frame = tms->last_frame = tms->irq_pin = 0;
  my_irq(0);
  ...
  tms->old_energy = tms->new_energy = tms->current_energy = tms->target_energy =
      0;
...
void tms5220_data_write(struct tms5220 *chip, int data) {
  ...
  if (!tms->speak_external)
    /* R Nabet : we parse commands at once.  It is necessary for such commands
     * as read. */
...
From the data sheet:
bit 0 = TS - Talk Status is active (high) when the VSP is processing speech
data. Talk Status goes active at the initiation of a Speak command or after nine
...
bit 1 = BL - Buffer Low is active (high) when the FIFO buffer is more than half
empty. Buffer Low is set when the "Last-In" byte is shifted down past the
half-full boundary of the stack. Buffer Low is cleared when data is loaded to
the stack so that the "Last-In" byte lies above the half-full boundary and
becomes the ninth data byte of the stack. bit 2 = BE - Buffer Empty is active
(high) when the FIFO buffer has run out of data while executing a Speak External
command. Buffer Empty is set when the last bit of the "Last-In" byte is shifted
out to the Synthesis Section. This causes Talk Status to be cleared. Speed is
terminated at some abnormal point and the Speak External command execution is
terminated.
  ...
  if (tms->RDB_flag) { /* if last command was read, return data register */
    ...
    if (DEBUG_5220)
      logerror("Status read: TS=%d BL=%d BE=%d\n", tms->talk_status,
               tms->buffer_low, tms->buffer_empty);

    return (tms->talk_status << 7) | (tms->buffer_low << 6) |
           (tms->buffer_empty << 5);
  }
}
...
process_command(tms);*/
  ...
  while ((size > 0) && tms->talk_status) {
    ...
    if ((tms->interp_count == 0) && (tms->sample_count == 0)) {
      ...
      tms->current_energy += (tms->target_energy - tms->current_energy) /
                             interp_coeff[interp_period];
      if (tms->old_pitch != 0)
        tms->current_pitch += (tms->target_pitch - tms->current_pitch) /
                              interp_coeff[interp_period];

      /*printf("*** Energy = %d\n",tms->current_energy);*/

      for (i = 0; i < 10; i++) {
        tms->current_k[i] += (tms->target_k[i] - tms->current_k[i]) /
                             interp_coeff[interp_period];
      }
    }
    ...
    if (tms->old_energy == 0) {
      ...
      if (tms->pitch_count < (int)sizeof(chirptable))
        //<-
        current_val =
            (chirptable[tms->pitch_count] * tms->current_energy) / 256;
      else
        current_val = 0x00;
    }
    ...
    for (i = 9; i >= 1; i--) {
      tms->x[i] =
          tms->x[i - 1] + ((tms->current_k[i - 1] * tms->u[i - 1]) / 32768);
    }
...
empty:
  ...
  while (size > 0) {
    tms->sample_count = (tms->sample_count + 1) % 200;
    tms->interp_count = (tms->interp_count + 1) % 25;
    buffer[buf_count] = 0x80; /* should be (-1 << 8) ??? (cf note in data sheet,
                                 p 10, table 4) */
    buf_count++;
    size--;
  }
...
static void process_command(struct tms5220 *tms) {
  ...
  if (tms->fifo_count > 0) {
    ...
    case 0x40: /* load address */
      /* tms5220 data sheet says that if we load only one 4-bit nibble, it won't
      work. This code does not care about this. */
    ...
    case 0x60: /* speak external */
      ...
      /* according to the datasheet, this will cause an interrupt due to a BE
       * condition */
      if (!tms->buffer_empty) {
        tms->buffer_empty = 1;
        set_interrupt_state(tms, 1);
...
static int extract_bits(struct tms5220 *tms, int count) {
  ...
  if (tms->speak_external) {
    ...
    while (count--) {
      val = (val << 1) |
            ((tms->fifo[tms->fifo_head] >> tms->fifo_bits_taken) & 1);
      tms->fifo_bits_taken++;
...
parse_frame -- parse a new frame's worth of data; returns 0 if not enough bits
in buffer
...
ranout:
  ...
  tms->buffer_empty = 1;
  tms->talk_status = tms->speak_external = tms->tms5220_speaking =
      the_first_frame = tms->last_frame = 0;
