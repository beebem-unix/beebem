...
void LoadCSW(char *file) {
  ...
  int sample_rate = file_buf[0x19] + (file_buf[0x1a] << 8) +
                    (file_buf[0x1b] << 16) + (file_buf[0x1c] << 24);
  int total_samples = file_buf[0x1d] + (file_buf[0x1e] << 8) +
                      (file_buf[0x1f] << 16) + (file_buf[0x20] << 24);
...
int csw_data(void) {
  ...
  if (csw_buff[csw_ptr] == 0) {
    if (csw_ptr + 4 < (int)csw_bufflen) {
      csw_ptr++;
      csw_pulselen = csw_buff[csw_ptr] + (csw_buff[csw_ptr + 1] << 8) +
