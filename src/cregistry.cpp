...
// This class provides easier way of accessing the registry from a c++
// application. Written by David Overton (david@overton.org.uk -- NO SPAM). Use
// this code if you want to, i found it much easier to use the registry with it.
...
//
// * The string/binary functions need to be modified to allow more than
// MAX_BUFFER_SIZE
//   bytes to be read in at any one time (possibly pass buffer size as function
//   param).
//
...
bool cRegistry::DeleteValue(HKEY hKeyRoot, LPCSTR lpSubKey,
                            LPCSTR lpValueName) {
  //+>
  //## The Windows version of Beebem does not call this function.
  //## hKeyRoot = User
...
bool cRegistry::GetBinaryValue(HKEY hKeyRoot, LPCSTR lpSubKey, LPCSTR lpValue,
  ...
  //--	DWORD dwSize	= *pnSize;
  //--	LONG  lRes		= 0;
  //--	if((RegOpenKeyEx(hKeyRoot, lpSubKey, 0, KEY_ALL_ACCESS,
...
bool cRegistry::GetDWORDValue(HKEY hKeyRoot, LPCSTR lpSubKey, LPCSTR lpValue,
                              DWORD &dwBuffer) {
  //+>
  // hKeyRoot = User
  // lpSubKey = Application ("Software\BeebEm")
  ...
  //--	if((RegOpenKeyEx(hKeyRoot, lpSubKey, 0, KEY_ALL_ACCESS,
  //&hKeyResult))==ERROR_SUCCESS) {
  //--		if((lRes=RegQueryValueEx(hKeyResult,lpValue, NULL, &dwType,
  // pBytes, &dwBufferSize))==ERROR_SUCCESS) {
  //--			if(dwType==REG_DWORD) {
...
bool cRegistry::GetStringValue(HKEY hKeyRoot, const LPCSTR lpSubKey,
                               LPCSTR lpValue, LPSTR lpBuffer) {
  //+>
  // hKeyRoot = User
  // lpSubKey = Application ("Software\BeebEm")
  // lpValue = value (all we need to concern ourselves with - string ptr).
  // lpBuffer = pointer to where string should be copied to (does not seem to
  // specify a length).
...
bool cRegistry::SetBinaryValue(HKEY hKeyRoot, LPCSTR lpSubKey, LPCSTR lpValue,
                               PVOID pData, int *pnSize) {
  //+>
  // hKeyRoot = User
  // lpSubKey = Application ("Software\BeebEm")
...
bool cRegistry::SetDWORDValue(HKEY hKeyRoot, LPCSTR lpSubKey, LPCSTR lpValue,
                              DWORD dwValue) {
  //+>
  // hKeyRoot = User
  // lpSubKey = Application ("Software\BeebEm")
...
bool cRegistry::SetStringValue(HKEY hKeyRoot, LPCSTR lpSubKey, LPCSTR lpValue,
                               LPCSTR lpData) {
  //+>
  // hKeyRoot = User
  // lpSubKey = Application ("Software\BeebEm")
