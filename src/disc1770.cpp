...
unsigned char FDCommand = 0, NFDCommand = 0; // NFD is used as "Next command"
                                             // during spin up/settling periods
...
bool InvertTR00; // Needed because the bloody stupid watford board inverts the
                 // input.
...
static void SetStatus(unsigned char bit) { Status |= 1 << bit; }

static void ResetStatus(unsigned char bit) { Status &= ~(1 << bit); }
  ...
  if (Register == 0) {
    ...
    if (HComBits < 0x80) {
      ...
      if (FDCommand < 6) {
        ...
        ESpinUp = (Value & 8);
        EVerify = (Value & 4);
        CStepRate = StepRate[(Value & 3)]; // Make sure the step rate time is
    ...
    if (*CDiscOpen && Sector > (RotSect + 1))
      SectorCycles =
          ((ONE_REV_TIME) / MaxSects[CurrentDrive]) * ((RotSect + 1) - Sector);
    ...
    if (ComBits == 0xD0) {
      ...
      Data =
          0; // If this isn't done, the stupid Opus DDOS tries to use the last
    ...
    if (ComBits == 0xc0) {
      ...
      if (!(Status & 128)) {
        ...
        FDCommand = 11; /* Spin-Up delay */
        LoadingCycles = SPIN_UP_TIME;
        // if (!ESpinUp) LoadingCycles=ONE_REV_TIME; // Make it two seconds
        // instead of one
        SetMotor(CurrentDrive, TRUE);
        SetStatus(7);
      } else {
...
void Poll1770(int NCycles) {
  ...
  // This procedure is called from the 6502core to enable the chip to do stuff
  // in the background
  if ((Status & 1) && (NMILock == 0)) {
    if (FDCommand < 6 && *CDiscOpen) {
      int TracksPassed =
          0; // Holds the number of tracks passed over by the head during seek
      ...
      if (FDCommand == 2) {
        fseek(CurrentDisc,
              DiscStrt[CurrentDrive] + (DiscStep[CurrentDrive] * Data),
              SEEK_SET);
        Track = Data;
      } // Seek
      if (FDCommand == 4) {
      ...
      if (FDCommand == 3) {
        fseek(CurrentDisc,
              (HeadDir) ? DiscStep[CurrentDrive] : -DiscStep[CurrentDrive],
              SEEK_CUR);
    ...
    if ((FDCommand >= 8) && (*CDiscOpen == 1) &&
        (FDCommand <= 10)) { // Read/Write Prepare
      SetStatus(0);
      ResetStatus(5);
      ResetStatus(6);
      ResetStatus(2);
      ...
      LoadingCycles = 45;
      fseek(CurrentDisc,
            DiscStrt[CurrentDrive] + (DiscStep[CurrentDrive] * Track) +
                (Sector * SecSize[CurrentDrive]),
            SEEK_SET);
    }
  ...
  if ((FDCommand >= 20) && (*CDiscOpen == 1) &&
      (FDCommand <= 21)) { // Read/Write Track Prepare
    SetStatus(0);
    ResetStatus(5);
    ResetStatus(6);
    ResetStatus(2);
    LoadingCycles = 45;
    fseek(CurrentDisc,
          DiscStrt[CurrentDrive] + (DiscStep[CurrentDrive] * Track), SEEK_SET);
    Sector = 0;
    ByteCount = 0;
    DataPos = ftell(CurrentDisc);
    HeadPos[CurrentDrive] = DataPos;
    //        fprintf(tlog, "Read/Write Track Prepare - Disc = %d, Track =
    //        %d\n", CurrentDrive, Track);
  }
...
unsigned char ReadFDCControlReg(void) { return (ExtControl); }
