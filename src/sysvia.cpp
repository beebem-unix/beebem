...
void DoKbdIntCheck() {
  ...
  /* Two cases - write enable is OFF the keyboard - basically any key will cause
     an interrupt in a few cycles.
     */
#ifdef KBDDEBUG
...
#ifdef KBDDEBUG
  cout << "DoKbdIntCheck KeysDown=" << KeysDown
       << "pcr & c=" << (int)(SysVIAState.pcr & 0xc);
  cout << " IC32State & 8=" << (int)(IC32State & 8) << " KBDRow=" << KBDRow
       << "KBDCol=" << KBDCol;
...
static int KbdOP(void) {
  ...
  if ((KBDCol > 9) || (KBDRow > 7))
    return (0); /* Key not down if overrange - perhaps we should do something
                   more? */
...
static void IC32Write(unsigned char Value) {
  ...
  // Additional, Sunday 4th February 2001. I must have been potty. the line
  // above did read January 2000.
  int bit;
  ...
  /* hmm, CMOS RAM? */
  // Monday 5th February 2001 - Scrapped my CMOS code, and restarted as
  // according to the bible of the god Tom Lees
  ...
  // now, this was a change from 0 to 1, but my docs say its a change from 1 to
  // 0. might work better this way.
  //--#endif
...
static void SlowDataBusWrite(unsigned char Value) {
  ...
  /*cerr << "Slow data bus write IC32State=" << int(IC32State) << " Value=" <<
   * int(Value) << "\n";*/
  if (!(IC32State & 8)) {
    KBDRow = (Value >> 4) & 7;
    KBDCol = (Value & 0xf);
...
static int SlowDataBusRead(void) {
  ...
  if (CMOS.Enabled && CMOS.Op && MachineType == 3) {
    SysVIAState.ora =
        CMOSRead(CMOS.Address); // SysVIAState.ddra ^ CMOSRAM[CMOS.Address];
    if (CMOSDebug)
      fprintf(CMDF, "Read %02x from %02x\n", SysVIAState.ora, CMOS.Address);
  }
...
void SysVIAWrite(int Address, int Value) {
  // fprintf(vialog,"SYSTEM VIA Write of %d (%02x) to address
  // %d\n",Value,Value,Address);
  /* cerr << "SysVIAWrite: Address=0x" << hex << Address << " Value=0x" << Value
  << dec << " at " << TotalCycles << "\n"; DumpRegs(); */

  if (DebugEnabled) {
    char info[200];
    sprintf(info, "SysVia: Write address %X value %02X", (int)(Address & 0xf),
            Value & 0xff);
  ...
  case 0:
    ...
    CMOS.Address = (((Value & 128) >> 7)) ? SysVIAState.ora
                                          : CMOS.Address; // CMOS Address strobe
...
int SysVIARead(int Address) {
  ...
  /* cerr << "SysVIARead: Address=0x" << hex << Address << dec << " at " <<
  TotalCycles << "\n"; DumpRegs(); */
  switch (Address) {
  case 0: /* IRB read */
          // Clear bit 4 of IFR from ATOD Conversion
  ...
  if (DebugEnabled) {
    char info[200];
    sprintf(info, "SysVia: Read address %X value %02X", (int)(Address & 0xf),
            tmp & 0xff);
    DebugDisplayTrace(DEBUG_SYSVIA, true, info);
  }
...
void SysVIAReset(void) {
  ...
  SRMode = 0;
  SRCount = 0;
  SREnabled = 0; // Disable Shift register shifting shiftily. (I am nuts) -
                 // Richard Gellman
} /* SysVIAReset */
...
void DebugSysViaState() { DebugViaState("SysVia", &SysVIAState); }
