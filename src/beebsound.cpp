...
// 14/04/01 - Proved that I AM better than DirectSound, by fixing the code
// thereof ;P
...
volatile struct BeebState {
  ...
  unsigned int
      ChangeSamps[4]; /* How often this channel should flip its otuput */
...
bool ReloadingChip;
static int ActiveChannel[4] = {FALSE, FALSE, FALSE,
                               FALSE}; /* Those channels with non-0 voolume */
...
__int64 SoundTrigger2,
    STCycles;      // BBC's Clock count for triggering host clock adjust
  ...
  while (DestTime > OurTime) {
    for (bufinc = 0;
        ...
        for (channel = 1; channel <= 3; channel++) {
          if (ActiveChannel[channel]) {
            ...
            if (!PartSamples)
              tt = 0;
            if (GenIndex[channel] >=
                (BeebState76489.ChangeSamps[channel] + tt)) {
              if (CSC[channel] >= 1.0)
                CSC[channel] -= 1.0;
        ...
        if (ActiveChannel[0]) {
          if (BeebState76489.Noise.FB) {
            ...
            case 3: /* Tone gen 1 */
              if (GenState[0]) {
                if (GenIndex[0] >=
                    ((BeebState76489.ChangeSamps[1] + (int)CSC[1]) * 31)) {
                  GenIndex[0] = 0;
                  GenState[0] = 0;
                }
              } else {
                if (GenIndex[0] >=
                    ((BeebState76489.ChangeSamps[1] + (int)CSC[1]))) {
                  GenIndex[0] = 0;
                  GenState[0] = 1;
                }
              }
      ...
      if (TapeSoundEnabled) {
        ...
        if ((TapeAudio.Enabled) && (TapeAudio.Signal == 1)) {
          tapetotal = (int)(sin(((TapeAudio.Samples *
                                  (10 * (1 + TapeAudio.CurrentBit))) *
                                 3.14) /
                                180) *
                            (80 + (40 * (1 - TapeAudio.CurrentBit))));
          // And if you can follow that equation, "ill give you the money
          // meself" - Richard Gellman
          if (TapeAudio.Samples++ >= 36) {
            TapeAudio.Samples = 0;
            TapeAudio.BytePos++;
    ...
    if (bufptr == SoundBufferSize) {
      ...
      if (fd != NULL) {
        ...
        MessageBox(GETHWND, "Failed to open audio.dbg", "BBC Emulator",
                   MB_OK | MB_ICONERROR);
        exit(1);
      }
#else
...
static double CyclesToSamples(__int64 beebtime) {
  ...
  /* This is all done incrementally - find the number of ticks since the last
     call in both domains.  This does mean this should only be called once */
...
static void SetFreq(int Channel, int freqval) {
  ...
  ChangeSamps =
      (int)((((double)samplerate / (double)freq) / 2.0) + SoundTuning);
...
void SoundChipReset(void) {
  ...
  BeebState76489.ToneVolume[1] = BeebState76489.ToneVolume[2] =
      BeebState76489.ToneVolume[3] = GetVol(15);
  BeebState76489.ToneFreq[0] = BeebState76489.ToneFreq[1] =
      BeebState76489.ToneFreq[2] = 1000;
...
void Sound_RegWrite(int value) {
  ...
  if (!(value & 0x80)) {
    ...
    SetFreq(BeebState76489.LastToneFreqSet + 1,
            BeebState76489.ToneFreq[BeebState76489.LastToneFreqSet]);
    trigger = 1;
  } else {
    /* Another register */
    VolChange = 0xff;
    switch ((value >> 4) & 0x7) {
    case 0: /* Tone 3 freq */
      BeebState76489.ToneFreq[2] =
          (BeebState76489.ToneFreq[2] & 0x2f0) | (value & 0xf);
    ...
    case 2: /* Tone 2 freq */
      BeebState76489.ToneFreq[1] =
          (BeebState76489.ToneFreq[1] & 0x2f0) | (value & 0xf);
    ...
    case 4: /* Tone 1 freq (Possibly also noise!) */
      BeebState76489.ToneFreq[0] =
          (BeebState76489.ToneFreq[0] & 0x2f0) | (value & 0xf);
      BeebState76489.LastToneFreqSet = 0;
    ...
    // if (VolChange<4) fprintf(sndlog,"Channel %d - Volume %d at %lu
