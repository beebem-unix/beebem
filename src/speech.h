...
#define FIFO_SIZE 16
  ...
  /*
   R Nabet : speak_external is only set when a speak external command is going
   on. tms5220_speaking is set whenever a speak or speak external command is
   going on. Note that we really need to do anything in tms5220_process and play
   samples only when tms5220_speaking is true.  Else, we can play nothing as
   well, which is a speed-up...
   */
  ...
  UINT8 first_frame;  /* we have just started speaking, and we are to parse the
                         first frame */
...
void tms5220_process(struct tms5220 *chip, short int *buffer,
                     unsigned int size);
