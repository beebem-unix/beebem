...
#define ADRMASK                                                                \
  (IMM | ABS | ACC | IMP | INX | INY | ZPX | ABX | ABY | REL | IND | ZPY |     \
   ZPG | ILL)
...
void DebugExecuteCommand() {
  ...
  //--
  //--				// Check if BP in list
  //--				i = (int)SendMessage(hwndBP, LB_FINDSTRING, 0,
  //(LPARAM)info);
  //--				if (i != LB_ERR)
  //--				{
  ...
  //--						BPCount++;
  //--					}
  //--					SendMessage(hwndBP, LB_ADDSTRING, 0,
  //(LPARAM)info);
  //--				}
  //--
...
int DebugDisassembleInstruction(int addr, bool host, char *opstr) {
  ...
  case 1:
    ...
    break;
  case 2:
    sprintf(s, "%02X %02X     ", DebugReadMem(addr, host),
            DebugReadMem(addr + 1, host));
    break;
  case 3:
    sprintf(s, "%02X %02X %02X  ", DebugReadMem(addr, host),
            DebugReadMem(addr + 1, host), DebugReadMem(addr + 2, host));
    break;
  }
  ...
  while (count > 0 && addr <= 0xffff) {
    if ((TorchTube || AcornZ80) && !host) {
      ...
      case 3:
        ...
        break;
      case 4:
        sprintf(s, "%02X %02X %02X %02X  ", DebugReadMem(addr, host),
                DebugReadMem(addr + 1, host), DebugReadMem(addr + 2, host),
                DebugReadMem(addr + 3, host));
        break;
      }
