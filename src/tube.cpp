...
unsigned char TubeintStatus =
    0; /* bit set (nums in IRQ_Nums) if interrupt being caused */
unsigned char TubeNMIStatus =
    0; /* bit set (nums in NMI_Nums) if NMI being caused */
...
/* Note how GETCFLAG is special since being bit 0 we don't need to test it to
 * get a clean 0/1 */
...
unsigned char ReadTorchTubeFromHostSide(unsigned char IOAddr) {
  ...
  if (!TorchTubeActive)
    return (MachineType == 3 ? 0xff
                             : 0xfe); // return ff for master else return fe
  ...
  if (DebugEnabled) {
    char info[200];
    sprintf(info, "Tube: Read from host, addr %X value %02X", (int)IOAddr,
            (int)TmpData);
...
void WriteTorchTubeFromHostSide(unsigned char IOAddr, unsigned char IOData) {
  ...
  //	fprintf(tlog, "WriteTorchTubeFromHostSide - Addr = %02x, Value =
  //%02x\n", (int) IOAddr, (int) IOData);

  if (DebugEnabled) {
    char info[200];
    sprintf(info, "Tube: Write from host, addr %X value %02X", (int)IOAddr,
            (int)IOData);
    DebugDisplayTrace(DEBUG_TUBE, true, info);
  }
...
unsigned char ReadTorchTubeFromParasiteSide(unsigned char IOAddr) {
  ...
  //	fprintf(tlog, "ReadTorchTubeFromParasiteSide - Addr = %02x, Value =
  //%02x\n", (int) IOAddr, (int) TmpData);

  if (DebugEnabled) {
    char info[200];
    sprintf(info, "Tube: Read from para, addr %X value %02X", (int)IOAddr,
            (int)TmpData);
...
void WriteTorchTubeFromParasiteSide(unsigned char IOAddr,
                                    unsigned char IOData) {

  //	fprintf(tlog, "WriteTorchTubeFromParasiteSide - Addr = %02x, Value =
  //%02x\n", (int) IOAddr, (int) IOData);

  if (DebugEnabled) {
    char info[200];
    sprintf(info, "Tube: Write from para, addr %X value %02X", (int)IOAddr,
            (int)IOData);
    DebugDisplayTrace(DEBUG_TUBE, false, info);
  }
...
/* Get a two byte address from the program counter, and then post inc the
 * program counter */
...
INLINE void Carried() {
  ...
  if (((CurrentInstruction & 0xf) == 0x1 || (CurrentInstruction & 0xf) == 0x9 ||
...
INLINE int SignExtendByte(signed char in) {
  ...
  /* I think this should sign extend by virtue of the casts - gcc does anyway -
  the code above will definitly do the trick */
  return ((int)in);
} /* SignExtendByte */
...
INLINE static void SetPSR(int mask, int c, int z, int i, int d, int b, int v,
                          int n) {
  PSR &= ~mask;
...
INLINE static void SBCInstrHandler(int16 operand) {
  ...
  if (!GETDFLAG) {
    TmpResultV =
        (signed char)Accumulator - (signed char)operand - (1 - GETCFLAG);
    TmpResultC = Accumulator - operand - (1 - GETCFLAG);
    Accumulator = TmpResultC & 255;
    SetPSR(FlagC | FlagZ | FlagV | FlagN, TmpResultC >= 0, Accumulator == 0, 0,
           0, 0, ((Accumulator & 128) > 0) ^ ((TmpResultV & 256) != 0),
           (Accumulator & 128));
  } else {
...
INLINE static int16 IndXAddrModeHandler_Data(void) {
  ...
  EffectiveAddress =
      TubeRam[ZeroPageAddress] | (TubeRam[ZeroPageAddress + 1] << 8);
...
INLINE static int16 ZPIndAddrModeHandler_Address(void) {
  ...
  EffectiveAddress =
      TubeRam[VectorLocation] + (TubeRam[VectorLocation + 1] << 8);
...
void Reset65C02(void) {
  ...
  // The fun part, the tube OS is copied from ROM to tube RAM before the
  // processor starts processing This makes the OS "ROM" writable in effect, but
  // must be restored on each reset.
...
void DoTubeNMI(void) {
  ...
  SetPSR(FlagI, 0, 0, 1, 0, 0, 0,
         0); /* Normal interrupts should be disabled during NMI ? */
  IRQCycles = 7;
} /* DoNMI */
...
void Exec65C02Instruction(void) {
  ...
  // cout << "Fetch at " << hex << (TubeProgramCounter-1) << " giving 0x" <<
  ...
  case 0x64:
    if (TubeMachineType == 3)
      TUBEWRITEMEM_DIRECT(ZeroPgAddrModeHandler_Address(),
                          0); /* STZ Zero Page */
    break;
  case 0x65:
  ...
  case 0xcb:
    ...
    // Subtract #n from X with carry.
    {
      ...
      XReg = Accumulator;
      Accumulator =
          TmpAcc; // Fudge so that I dont have to do the whole SBC code again
    }
    break;
  default:
...
/*-------------------------------------------------------------------------*/
void WrapTubeCycles(void) { TotalTubeCycles -= CycleCountWrap / 2 * 3; }
...
void DebugTubeState(void) {
  ...
  sprintf(info, "HostTube: R1=%02X R2=%02X R3=%02X R4=%02X R1n=%02X R3n=%02X",
          (int)R1HStatus | R1Status, (int)R2HStatus, (int)R3HStatus,
          (int)R4HStatus, (int)R1PHPtr, (int)R3PHPtr);
  DebugDisplayInfo(info);

  sprintf(info, "ParaTube: R1=%02X R2=%02X R3=%02X R4=%02X R3n=%02X",
          (int)R1PStatus | R1Status, (int)R2PStatus, (int)R3PStatus,
          (int)R4PStatus, (int)R3HPPtr);
  DebugDisplayInfo(info);
}
...
void Load65C02MemUEF(FILE *SUEF) { fread(TubeRam, 1, 65536, SUEF); }
