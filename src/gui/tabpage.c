...
static EG_BOOL Callback_Paint(EG_Widget *widget_ptr, SDL_Rect area) {
   ...
   */
  if (EG_Shared_GetRenderingDetails(widget_ptr, area, (void *)&tabpage_ptr,
...
static EG_BOOL Callback_SDL_Event(EG_Widget *widget_ptr, SDL_Event *event_ptr) {
   ...
   * false.
   */
  if (EG_Shared_GetEventDetails(widget_ptr, event_ptr, (void *)&tabpage_ptr,
  ...
  if (event_ptr->type == SDL_MOUSEBUTTONDOWN ||
     ...
     */
    if (tabpage_ptr->depressed == EG_FALSE &&
        event_ptr->button.state == SDL_PRESSED &&
     ...
     */
    if (tabpage_ptr->depressed == EG_TRUE &&
        event_ptr->button.state == SDL_RELEASED &&
        event_ptr->button.button == SDL_BUTTON_LEFT) {
...
// [TODO] This is fixed now, (forgot to do nothing when detaching {we don't need
// to do anything for that in this version of EG as the window micro-manages
// everything, in the final version where parent widgets manage child widgets
// [thus keeping their implementation secret from the parent], so they will need
// to detatch their children from window resources themselves).
...
static void InitializePayload(EG_TabPage *tabpage_ptr,
...
//                EG_Log(EG_LOG_ERROR, dL"EG_Widget contains NULL payload.",
//                dR); return(EG_FALSE);
//        }
//
//	/* Add the page widgets to the parent window.
...
EG_Widget *EG_TabPage_Create(const char *name_ptr, const char *caption_ptr) {
  ...
  SHARED__CREATE_NEW_EG_WIDGET(widget_ptr, name_ptr, EG_Widget_Type_TabPage,
                               tabpage_ptr);
...
EG_BOOL EG_TabPage_AddWidget(EG_Widget *page_widget_ptr,
                             EG_Widget *child_widget_ptr) {
  EG_TabPage *tabpage_ptr;
...
// Think about a 'I'm registered with callback', that way stock functions can
// delete widgets and who they are registered with can be notified, so they can
// remove them from their list.  eg.  EG_Widget_Remove(whatever) -> registered
// with tabpage so tabpage is notified when widget is destroyed, so it knows to
// remove it from it's list.

void EG_TabPage_RemoveWidget(EG_Widget *page_widget_ptr,
                             EG_Widget *child_widget_ptr) {
  // [TODO] If this widget has focus, movefocus, if this widget still has focus,
  // clear focus.
...
EG_BOOL EG_TabPage_SetTabGroup(EG_Widget *page_widget_ptr,
