...
unsigned char HiddenDefault[31] = {0,    0,    0,    0,    0,    0,    2,    1,
                                   1,    0,    0xe0, 0x8e, 0,    0,    0,    0,
                                   0,    0,    0,    0xef, 0xff, 0xff, 0x78, 0,
                                   0x17, 0x23, 0x19, 5,    0x0a, 0x2d, 0xa0};
/* End of Computech (&B+) Specific Stuff */
...
static unsigned int WrapAddr(int in) {
  unsigned int offsets[] = {0x4000, 0x6000, 0x3000,
                            0x5800}; // page 419 of AUG is wrong
  if (in < 0x8000)
    return (in);
...
int BeebReadMem(int Address) {
  ...
  if (MachineType == 1) {
    if (Address < 0x3000)
      return (WholeRam[Address]);
    if ((Address >= 0x8000) && (Address < 0x8400) && (Prvs8 == 1) &&
        (PrvEn == 1))
      return (Private[Address - 0x8000]);
    if ((Address >= 0x8000) && (Address < 0x9000) && (Prvs4 == 1) &&
        (PrvEn == 1))
      return (Private[Address - 0x8000]);
    if ((Address >= 0x9000) && (Address < 0xb000) && (Prvs1 == 1) &&
        (PrvEn == 1))
      return (Private[Address - 0x8000]);
  ...
  if (MachineType == 2) {
    if (Address < 0x3000)
      return (WholeRam[Address]);
    if ((Address < 0x8000) && (Sh_Display == 1) && (PrePC >= 0xC000) &&
        (PrePC < 0xE000))
      return (ShadowRAM[Address]);
    if ((Address < 0x8000) && (Sh_Display == 1) && (MemSel == 1) &&
        (PrePC >= 0xA000) && (PrePC < 0xB000))
      return (ShadowRAM[Address]);
    if (Address < 0x8000)
      return (WholeRam[Address]);
  ...
  /* Rob: BBC AUG says FE20 is econet stn no. for bbc b. [It's in cmos for a
     master,] This is wrong and doesn't work. Study of the circuit diagram for a
     model B (IC26) shows it's at FE18-FE1F. This now works.  AUG says read
     station also enables Econet NMI but again the circuit shows the
     read-station select line is also INTOFF. (and it's any access to FE18, not
     just a read.)
  */
#ifdef WITH_ECONET
  if (EconetEnabled && ((MachineType != 3 && (Address & ~3) == 0xfe18) ||
  ...
  /* Rob: BBC AUG states video ULA at FE20-21 is write-only - why support
     reading? The circuit diagram shows read of FE20-FE27 is INTON (maybe this
     is where AUG confusion came from) INTON & INTOFF enable/disable NMI from
     the 68B54 via flip flop IC97, IC27 & IC91.  Guess what happens if the ADLC
     is holding an NMI which was masked by INTOFF and you turn INTON ....! (NMI
     from the FDC is always enabled)
  */
#ifdef WITH_ECONET
  if (EconetEnabled && ((MachineType != 3 && (Address & ~3) == 0xfe20) ||
  ...
  if ((Address & ~3) == 0xfe30) {
    return (
        PagedRomReg); /* report back ROMSEL - I'm sure the beeb allows ROMSEL
                         read.. correct me if im wrong. - Richard Gellman */
  }
  // In the Master at least, ROMSEL/ACCCON seem to be duplicated over a 4 byte
  // block.
  ...
  if ((Address & ~0x1f) == 0xfee0) {
    if (TorchTube)
      return (ReadTorchTubeFromHostSide(Address & 0x1f)); // Read From Torch
                                                          // Tube
    else
  ...
  if ((MachineType != 3) && (Address >= EFDCAddr) &&
      (Address < (EFDCAddr + 4)) && (!NativeFDC)) {
    // MessageBox(GETHWND,"Read of 1770 Extension
...
static void FiddleACCCON(unsigned char newValue) {
  ...
  if ((newValue & 128) == 128)
    DoInterrupt();
  ACCCON = newValue &
           127; // mask out the IRR bit so that interrupts dont occur repeatedly
    ...
    if (Address < 0x8000) {
      if ((Sh_Display == 1) && (PrePC >= 0xC000) && (PrePC < 0xE000)) {
        ...
        return;
          ...
          if ((Sh_Display == 1) && (MemSel == 1) && (PrePC >= 0xA000) &&
              (PrePC < 0xB000)) {
        ShadowRAM[Address] = Value;
        return;
      } else {
  ...
  if (RomCfg != NULL) {
    ...
    if (InFile != NULL) {
      if (fread(WholeRam + 0xc000, 1, 16384, InFile) < 16384) {
        pFATAL(dL "OS ROM [%s] load failed (file was too small)..", dR,
               fullname);
        exit(1);
      } else {
        char tmp[10];
        if (fread(tmp, 1, 1, InFile) == 0) {
          ...
          pWARN(dL "OS ROM [%s] did load, but this file is too large!", dR,
                fullname);
        }
      }
      fclose(InFile);
    ...
    for (romslot = 15; romslot >= 0; romslot--) {
      ...
      isrom = 1;
      RomWritable[romslot] = 0; // Shortener=1;
      ...
      if (isrom == 1) {
        ...
        if (InFile != NULL) {
          if (fread(Roms[romslot], 1, 16384, InFile) < 16384) {
            pFATAL(dL "Paged ROM 0x%X [%s] load failed (file is too small)..",
                   dR, romslot, fullname);
            exit(1);
          } else {
            char tmp[10];
            if (fread(tmp, 1, 1, InFile) == 0) {
              pINFO(dL "Paged ROM 0x%X [%s] loaded successfully.", dR, romslot,
                    RomName);
            } else {
              pWARN(dL
                    "Paged ROM 0x%X [%s] did load, but this file is too large!",
                    dR, romslot, fullname);
            }
          }
          ...
          pFATAL(dL "Paged ROM 0x%X [%s] load failed (unable to open file)..",
                 dR, romslot, fullname);
          exit(1);
        }
      } else {
...
void BeebMemInit(unsigned char LoadRoms, unsigned char SkipIntegraBConfig) {
  ...
  if ((CMDF3 = fopen(TmpPath, "rb")) != NULL) {
    ...
    if (CMOSLength == 50) {
      ...
      pERROR(dL "CMOS file %s is not 50 bytes long, guessing defaults instead!",
             dR, TmpPath);
    }
    fclose(CMDF3);
  } else
...
void LoadMainMemUEF(FILE *SUEF) { fread(WholeRam, 1, 32768, SUEF); }
...
void LoadFileMemUEF(FILE *SUEF) { fread(FSRam, 1, 8192, SUEF); }

void LoadIntegraBHiddenMemUEF(FILE *SUEF) { fread(Hidden, 1, 256, SUEF); }
