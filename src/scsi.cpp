  ...
  for (i = 0; i < 4; ++i) {
    ...
    if (SCSIDisc[i] != NULL) {
      ...
      if (f != NULL) {
        ...
        SCSISize[i] = buff[15] * (buff[13] * 256 + buff[14]) *
                      33; // Number of sectors on disk = heads * cyls * 33
...
int SCSIRead(int Address) {
  ...
  case 0x01:     // Status Register
    data = 0x20; // Hmmm.. don't know why req has to always be active ? If start
                 // at 0x00, ADFS lock up on entry
    if (scsi.cd)
      data |= 0x80;
    if (scsi.io)
      data |= 0x40;
...

{
  ...
  if (f != NULL) {
    ...
    SCSISize[scsi.lun] = buff[15] * (buff[13] * 256 + buff[14]) *
                         33; // Number of sectors on disk = heads * cyls * 33
