...

{
  ...
  sprintf(str + strlen(str), " BC=%04X DE=%04X HL=%04X", regs[0].bc, regs[0].de,
          regs[0].hl);
  sprintf(str + strlen(str), " IX=%04X I=%02X PC=%04X", ix, ir & 255, pc);
  sprintf(str + strlen(str), ":%02X,%02X,%02X,%02X", ReadZ80Mem(pc),
          ReadZ80Mem(pc + 1), ReadZ80Mem(pc + 2), ReadZ80Mem(pc + 3));
}
...

{
  ...
  sprintf(str + strlen(str), " BC'%04X DE'%04X HL'%04X", regs[1].bc, regs[1].de,
          regs[1].hl);
  sprintf(str + strlen(str), " IY=%04X R=%02x SP=%04X", iy, regs_sel, sp);
  sprintf(str + strlen(str), ":%02X,%02X,%02X,%02X\n", ReadZ80Mem(sp),
          ReadZ80Mem(sp + 1), ReadZ80Mem(sp + 2), ReadZ80Mem(sp + 3));
}
