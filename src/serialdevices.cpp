...
bool TouchScreenPoll(void) {
  ...
  if (ts_outlen >
      0) // Process data waiting to be received by BeebEm straight away
  {
    return true;
  }
  ...
  if (ts_inlen > 0) {
    ...
    case '.':
      ...
      //->				WriteLog("Setting touch screen mode to
      //%d\n", mode);
      //++
  ...
  if (mode == 130) // area mode - seems to be pressing with two fingers which we
                   // can't really do ??
  {
    TouchScreenReadScreen(true);
  }
...
void TouchScreenReadScreen(bool check) {
  ...
  if ((last_x != x) || (last_y != y) || (last_m != AMXButtons) ||
      (check == false)) {
