...
HRESULT AVIWriter::Initialise(const CHAR *pszFileName,
                                                          const WAVEFORMATEX
*WaveFormat, const bmiData *BitmapFormat, int fps, HWND hWnd)
{
        ...
        if (SUCCEEDED(hr) && WaveFormat)
        {
                ...
                hr = AVIFileCreateStream(m_pAVIFile, &m_pAudioStream,
&StreamInfo);
        }
        ...
        //
        //	hr = AVIMakeCompressedStream(&m_pCompressedAudioStream,
m_pAudioStream, &opts, NULL);
        //}
...
+ m_WaveFormat.cbSize);
        ...
        if (SUCCEEDED(hr))
        {
                ...
                StreamInfo.dwSuggestedBufferSize =
m_BitmapFormat.bmiHeader.biSizeImage; StreamInfo.dwQuality = (DWORD)-1;
                ...
                hr = AVIFileCreateStream(m_pAVIFile, &m_pVideoStream,
&StreamInfo);
        }
        ...
        if (SUCCEEDED(hr))
        {
                ...
                opts.fccHandler = mmioFOURCC('m', 'r', 'l', 'e'); // Microsoft
RLE
                //opts.fccHandler = mmioFOURCC('m', 's', 'v', 'c'); // Microsoft
Video 1 opts.dwKeyFrameEvery = 100; opts.dwQuality = 0; opts.dwBytesPerSecond =
0; opts.dwFlags = AVICOMPRESSF_KEYFRAMES; opts.lpFormat = 0; opts.cbFormat = 0;
                opts.lpParms = 0;
                opts.cbParms = 0;
                opts.dwInterleaveEvery = 0;
                ...
                hr = AVIMakeCompressedStream(&m_pCompressedVideoStream,
m_pVideoStream, &opts, NULL);
                                                          ...
                                                          ULONG nBytesToWrite)
{
        ...
        while ((hr == S_OK) && (nTotalBytesWritten < nBytesToWrite))
        {
                ...
                hr = AVIStreamWrite(m_pAudioStream,
                                                        -1,
// append at the end of the stream nSamples2Write,	 // how many samples to
write pBuffer,		 // where the data is
                                                        nBytesToWrite, // how
much data do we have AVIIF_KEYFRAME,	 // self-sufficient data
                                                        &nSamplesWritten,// how
many samples were written &nBytesWritten); // how many bytes were written
...
HRESULT AVIWriter::WriteVideo(BYTE *pBuffer)
{
        ...
        hr = AVIStreamWrite(m_pCompressedVideoStream,
                                                m_nFrame,        // append at
the end of the stream 1,               // how many samples to write pBuffer, //
where the data is m_BitmapFormat.bmiHeader.biSizeImage, // how much data do we
have AVIIF_KEYFRAME,	 // self-sufficient data &nSamplesWritten,// how many
samples were written &nBytesWritten); // how many bytes were written if
(SUCCEEDED(hr)) m_nFrame++;
