...
static EG_BOOL Callback_Paint(EG_Widget *widget_ptr, SDL_Rect area) {
   ...
   */
  if (EG_Shared_GetRenderingDetails(widget_ptr, area, (void *)&tabgroup_ptr,
  ...
  // [TODO] Change this, it'll leave a line around the box. Look into the border
  // flicker thing.
...
static EG_BOOL Callback_Stopped(EG_Widget *widget_ptr, EG_BOOL is_stopped) {
  ...
  for (i = 0; i < tabgroup_ptr->count; i++) {
    ...
    //			EG_Widget_SetStoppedToggle(tabgroup_ptr->page_widget_ptr[i],
    // is_stopped);
  }
...
static void InitializeWidget(EG_Widget *widget_ptr, SDL_Color color,
  ...
  (void)EG_Widget_SetCallback_LostFocus(
      widget_ptr, EG_Callback_Generic_LostFocus_NoSupport);
...
EG_Widget *EG_TabGroup_Create(const char *name_ptr, SDL_Color color,
  ...
  SHARED__CREATE_NEW_EG_WIDGET(widget_ptr, name_ptr, EG_Widget_Type_TabGroup,
                               tabgroup_ptr);
...
#define SPACER 2
   ...
   */
  if (EG_Widget_SetBackgroundColor(
          page_widget_ptr, EG_Widget_GetBackgroundColor(group_widget_ptr)) !=
      EG_TRUE)
    EG_Log(EG_LOG_ERROR,
...
void EG_TabGroup_RemovePage(EG_Widget *group_widget_ptr,
   ...
   */
  for (i = 0; i < tabgroup_ptr->count; i++)
    if (EG_Widget_GetID(page_widget_ptr) ==
        EG_Widget_GetID(tabgroup_ptr->page_widget_ptr[i]))
      break;
  ...
  // [TODO] Set tabgroups page to page after removed page, if we deleted the
  // last page, set tabgroups page to the previous page.  If this was the only
  // page, render only the tabgroup (will paint background). [TODO] If a widget
  // on the page had focus before we removed it, set focus to the next page.  If
  // we removed the last page, set focus to the previous page, if we deleted the
  // only page, ask window to movefocus.  (will set to first widget).
...
EG_BOOL EG_TabGroup_SelectPageByIndex(EG_Widget *widget_ptr, int page) {
  ...
  /* [TODO] Clear the windows focus otherwise repainting removes tail of button
   * thats just lost focus..  This should be fixed..
   */
  ...
  if (tabgroup_ptr->selected >= 0) {
    ...
    if (EG_TabPage_Hide(
            tabgroup_ptr->page_widget_ptr[tabgroup_ptr->selected]) != EG_TRUE) {
...
EG_Widget *EG_TabGroup_GetSelectedPageWidget(EG_Widget *widget_ptr) {
  ...
  if (tabgroup_ptr->selected < 0 ||
      tabgroup_ptr->selected >= MAX_TABGROUP_PAGES)
    return (NULL);
  else
