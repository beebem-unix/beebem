...
#define DefaultBase(Seg)                                                       \
  ((seg_prefix && (Seg == DS || Seg == SS)) ? prefix_base : I.base[Seg])

#define GetMemB(Seg, Off)                                                      \
  (program_read_byte_8((DefaultBase(Seg) + (Off)) & AMASK))
#define GetMemW(Seg, Off)                                                      \
  ((WORD)GetMemB(Seg, Off) + (WORD)(GetMemB(Seg, (Off) + 1) << 8))
#define PutMemB(Seg, Off, x)                                                   \
  program_write_byte_8((DefaultBase(Seg) + (Off)) & AMASK, (x))
...
#define ReadWord(ea)                                                           \
  (program_read_byte_8((ea)&AMASK) +                                           \
   (program_read_byte_8(((ea) + 1) & AMASK) << 8))
...
#define CompressFlags()                                                        \
  (WORD)(CF | (PF << 2) | (AF << 4) | (ZF << 6) | (SF << 7) | (I.TF << 8) |    \
         (I.IF << 9) | (DF << 10) | (OF << 11))
