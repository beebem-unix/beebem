...
void VIAReset(VIAState *ToReset) {
  ...
  ToReset->acr =
      0; /* Timed ints on t1, t2, no pb7 hacking, no latching, no shifting */
...
void DebugViaState(const char *s, VIAState *v) {
  ...
  sprintf(info, "%s: ora=%02X orb=%02X ira=%02X irb=%02X ddra=%02X ddrb=%02X",
          s, (int)v->ora, (int)v->orb, (int)v->ira, (int)v->irb, (int)v->ddra,
          (int)v->ddrb);
  DebugDisplayInfo(info);

  sprintf(info, "%s: acr=%02X pcr=%02X ifr=%02X ier=%02X", s, (int)v->acr,
          (int)v->pcr, (int)v->ifr, (int)v->ier);
  DebugDisplayInfo(info);

  sprintf(
      info, "%s: t1=%04X%s t2=%04X%s t1l=%04X t2l=%04X t1s=%d t2s=%d", s,
      (int)(v->timer1c < 0 ? ((v->timer1c - 1) / 2) & 0xffff : v->timer1c / 2),
      v->timer1c & 1 ? "+" : " ",
      (int)(v->timer2c < 0 ? ((v->timer2c - 1) / 2) & 0xffff : v->timer2c / 2),
      v->timer2c & 1 ? "+" : " ", (int)v->timer1l, (int)v->timer2l,
