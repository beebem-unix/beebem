...
static float GetPositionAsFloat(EG_Widget *widget_ptr) {
  ...
  return (float)slidebar_ptr->position /
...
static void Calc_Slider_Area(EG_Widget *widget_ptr) {
  ...
  case EG_SlideBar_Vertical:
    ...
    slidebar_ptr->slider_area.y +=
        (slidebar_ptr->background_area.h - slidebar_ptr->slider_area.h) *
        GetPositionAsFloat(widget_ptr);
    break;
  ...
  case EG_SlideBar_Horizontal:
    ...
    slidebar_ptr->slider_area.x +=
        (slidebar_ptr->background_area.w - slidebar_ptr->slider_area.w) *
        GetPositionAsFloat(widget_ptr);
    break;
  }
}
...
static void Move_Slider(EG_Widget *widget_ptr) {
  ...
  case EG_SlideBar_Vertical:
     ...
     */
    scrollable_area_length =
        slidebar_ptr->background_area.h - slidebar_ptr->slider_area.h;

    /* Calc movement of slidebar within troff.
     */
    new_slider_position =
        (float)(slidebar_ptr->slider_area.y - slidebar_ptr->background_area.y) +
        slidebar_ptr->mouse_y;
    ...
    slidebar_ptr->slider_area.y =
        new_slider_position + slidebar_ptr->background_area.y;
    break;
  ...
  case EG_SlideBar_Horizontal:
     ...
     */
    new_slider_position =
        (float)(slidebar_ptr->slider_area.x - slidebar_ptr->background_area.x) +
        slidebar_ptr->mouse_x;
    ...
    slidebar_ptr->slider_area.x =
        new_slider_position + slidebar_ptr->background_area.x;
    break;
  }
...
static EG_BOOL Callback_SDL_Event(EG_Widget *widget_ptr, SDL_Event *event_ptr) {
   ...
   * false.
   */
  if (EG_Shared_GetEventDetails(widget_ptr, event_ptr, (void *)&slidebar_ptr,
                                &window_ptr, &mouse_over_widget) != EG_TRUE)
    return (return_value);
  ...
  if (EG_Shared_IsMouseOverArea(widget_ptr, slidebar_ptr->button1_area,
                                event_ptr) == EG_TRUE)
    mouse_is_over = OVER_BUTTON1;
  if (EG_Shared_IsMouseOverArea(widget_ptr, slidebar_ptr->background_area,
                                event_ptr) == EG_TRUE)
    mouse_is_over = OVER_TROFF;
  if (EG_Shared_IsMouseOverArea(widget_ptr, slidebar_ptr->slider_area,
                                event_ptr) == EG_TRUE)
    mouse_is_over = OVER_SLIDER;
  if (EG_Shared_IsMouseOverArea(widget_ptr, slidebar_ptr->button2_area,
                                event_ptr) == EG_TRUE)
  ...
  if (mouse_is_over == OVER_TROFF) {
    ...
    switch (slidebar_ptr->orientation) {
    case EG_SlideBar_Vertical:
      if (event_ptr->motion.y <
          slidebar_ptr->slider_area.y + EG_Window_GetYDisplacement(window_ptr))
    ...
    case EG_SlideBar_Horizontal:
      if (event_ptr->motion.x <
          slidebar_ptr->slider_area.x + EG_Window_GetXDisplacement(window_ptr))
        mouse_is_over = OVER_TROFF_AND_ABOVE_SLIDER;
      else
        mouse_is_over = OVER_TROFF_AND_BELOW_SLIDER;
      break;
  ...
  if (event_ptr->type == SDL_MOUSEBUTTONDOWN ||
    ...
    if (event_ptr->button.state == SDL_PRESSED &&
      ...
      if (mouse_is_over == OVER_BUTTON2 &&
        ...
        /*	Yes, set 'button2_depressed' to true, scroll by minus step
         *amount, repaint widget.
         */
  ...
  if (event_ptr->type == SDL_MOUSEMOTION &&
    ...
    case EG_SlideBar_Vertical:
      slidebar_ptr->mouse_y +=
          event_ptr->motion.yrel * (1 / EG_Draw_GetScale());
      break;

    case EG_SlideBar_Horizontal:
      slidebar_ptr->mouse_x +=
          event_ptr->motion.xrel * (1 / EG_Draw_GetScale());
      break;
    }
  ...
  /* [TODO] If have focus and 'Forward' or 'Backward' key pressed then process
   * it as if button1/2 were pressed.
   */
...
static EG_BOOL Callback_Paint(EG_Widget *widget_ptr, SDL_Rect area) {
  ...
  if (EG_Shared_GetRenderingDetails(widget_ptr, area, (void *)&slidebar_ptr,
                                    &window_ptr, &surface_ptr, &color,
                                    &loc) != EG_TRUE)
    return (EG_TRUE);
  ...
  printf("SOMEONE CALLED PAINT FOR '%s' [repaint area (window)="
         ...
         EG_Widget_GetName(widget_ptr), area.x, area.y, area.w, area.h, loc.x,
         loc.y, loc.w, loc.h);
#endif
  ...
  EG_Draw_String(surface_ptr, &color, bold, &tmp, 0,
                 slidebar_ptr->button1_label);
...
static void InitializePayload(EG_SlideBar *slidebar_ptr, SDL_Color color,
                              int orientation, SDL_Rect area) {
  switch (orientation) {
  case EG_SlideBar_Vertical:
...
EG_Widget *EG_SlideBar_Create(const char *name_ptr, SDL_Color color,
                              int orientation, SDL_Rect area) {
  EG_SlideBar *slidebar_ptr;
  ...
  SHARED__CREATE_NEW_EG_WIDGET(widget_ptr, name_ptr, EG_Widget_Type_SlideBar,
                               slidebar_ptr);
...
/* Set the visible length of the scrolling area (and also how fat the widget
 * is).
 */
...
/* Set the virtual length of the scrolling area. If the new length is less than
 * the visible length, then the virtual length is set to the visible length
 * instead.  So length=0 will reset the scroll bar to a 1:1 ratio.
 */
...
/* position is 0 to length - widget_length (if horizontal this is width, if
 * vertical this is height).
 */
...
/* get the start position within the virtual scrolling area.  Read this in an
 * on-change event.
 */
...
/* Sets the stepping value in pixels, button1 and button2 will scroll by this
 * amount.
 */
